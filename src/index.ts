...
export class InstagramManagement {
  ...
  async initialize() {
    ...
    this.currentUser = await this.instagram.account.login(
  ...
  findBestQuality(listOfVersions: UserStoryFeedResponseItemsItem[]) {
    return _.first(listOfVersions.sort((version: IImageDimensions) => {
  ...
  async saveStory(storyData: UserStoryFeedResponseItemsItem, baseFolder: any) {
    switch (storyData.media_type) {
      case 1:
        await this.saveFile(
          this.findBestQuality(storyData.image_versions2.candidates),
          baseFolder
        );
        break;
      case 2:
        await this.saveFile(
          this.findBestQuality(storyData.video_versions),
          baseFolder
        );
        break;
      default:
  ...
  async scrapeStories(username: string, baseFolder: string) {
    ...
    const storyFeed = this.instagram.feed.userStory(
      await this.instagram.user.getIdByUsername(username)
    );
